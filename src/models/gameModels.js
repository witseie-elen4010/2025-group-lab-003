...
exports.disableGame = async (gameCode) => {
  ...
  await pool.request().input('gameCode', db.sql.VarChar, gameCode).query(query);
};
...
exports.getPlayersByGameCode = async (gameCode) => {
  ...
  const result = await pool
    ...
    .input('gameCode', db.sql.VarChar, gameCode)
    .query(
      `SELECT userId, role, status FROM Players WHERE gameCode = @gameCode`
    );
...
exports.assignRoles = async (gameCode) => {
  ...
  for (let i = 0; i < players.length; i++) {
    await pool
      ...
      .input('gameCode', db.sql.VarChar, gameCode)
      .input('role', db.sql.VarChar, shuffledRoles[i]).query(`
...
async function getUsedRoundsPairs(pool, gameCode) {
  ...
  const result = await pool
    ...
    .input('gameCode', db.sql.VarChar, gameCode)
    .query(
      `SELECT undercoverWord, civilianWord FROM RoundWords WHERE gameCode = @gameCode`
    );
...
function getUnusedPairs(allPairs, usedPairs) {
  return allPairs.filter(
    (pair) =>
      !usedPairs.some(
        (used) =>
          used.undercoverWord === pair.undercoverWord &&
          used.civilianWord === pair.civilianWord
      )
  );
}
...
exports.assignWordsForRound = async (gameCode, round) => {
  ...
  await pool
    ...
    .input('civilianWord', db.sql.VarChar, selectedPair.civilianWord).query(`
  ...
  const playersResult = await pool
    ...
    .input('gameCode', db.sql.VarChar, gameCode)
    .query(
      `SELECT userId, role FROM Players WHERE gameCode = @gameCode AND status = 'active'`
    );
  ...
  for (const player of playersResult.recordset) {
    const word =
      player.role === 'undercover'
        ? selectedPair.undercoverWord
        : selectedPair.civilianWord;
    await pool
      .request()
      .input('userId', db.sql.VarChar, player.userId)
      .input('gameCode', db.sql.VarChar, gameCode)
      .input('word', db.sql.VarChar, word).query(`
...
exports.getPlayerByNameAndGameCode = async (name, gameCode) => {
  ...
  const result = await pool
    ...
    .input('gameCode', db.sql.VarChar, gameCode)
    .query(
      `SELECT word, role FROM Players WHERE userId = @userId AND gameCode = @gameCode`
    );
...
exports.getPlayerIdByUserId = async (gameCode, userId) => {
  ...
  const result = await pool
    ...
    .query(
      'SELECT id FROM Players WHERE gameCode = @gameCode AND userId = @userId'
    );
...
exports.endGame = async (gameCode, winnerSide) => {
  ...
  await pool
    ...
    .query(
      `UPDATE GameState SET winner = @winner, gameStarted = 0 WHERE gameCode = @gameCode`
    );
};
...
exports.saveChatMessage = async (gameCode, round, playerName, message) => {
  ...
  await pool
    ...
    .input('message', db.sql.NVarChar, message).query(`
  ...
  await logAction(
    playerName,
    'CHAT_MESSAGE',
    `In game ${gameCode}, round ${round}: ${message}`,
    gameCode
  );
};
...
exports.getGameState = async (gameCode) => {
  ...
  const result = await pool
    ...
    .input('gameCode', db.sql.VarChar, gameCode)
    .query(
      `SELECT winner, round, gameStarted, mode FROM GameState WHERE gameCode = @gameCode`
    );
  return result.recordset[0];
};
...
exports.getActivePlayers = async (gameCode) => {
  ...
  const result = await pool
    ...
    .input('gameCode', db.sql.VarChar, gameCode)
    .query(
      `SELECT userId, role, status FROM Players WHERE gameCode = @gameCode AND status = 'active'`
    );
  return result.recordset;
};
...
exports.haveEligibleVotersVoted = async (
  gameCode,
  round,
  eligibleVoterUserIds
) => {
  const db = require('../config/db');
  const pool = await db.poolPromise;
  ...
  const placeholders = eligibleVoterIds
    .map((_, index) => `@voterId${index}`)
    .join(',');
  const votesCountQuery = `
    SELECT COUNT(DISTINCT voterId) AS votesCount
    FROM Votes
  ...
  console.log(
    `Eligible voters check: ${votesCount}/${expectedVotes} eligible voters have voted`
  );
